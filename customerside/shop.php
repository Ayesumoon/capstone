      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        ...
        <?php if ($result->num_rows > 0): ?>
          <?php while($row = $result->fetch_assoc()): ?>
            <div class="bg-white rounded-xl shadow-md p-4 transform transition-transform duration-300 hover:scale-105 hover:-translate-y-2 hover:shadow-lg relative" 
                 x-data="{ addedToCart: false }" 
                 x-init="addedToCart = false">

              <?php 
                $price = number_format($row['price'], 2); // Formatting the price
                $productName = htmlspecialchars($row['product_name']);
                $productImage = $row['image_url'];
                $productId = $row['product_id'];
              ?>
              <a href="product_detail.php?product_id=<?= $productId ?>" class="block">
                <img src="<?= $productImage ?>" alt="<?= $productName ?>" class="w-full h-48 object-cover rounded-lg mb-4">
                <h4 class="text-lg font-semibold"><?= $productName ?></h4>
                <p class="text-sm text-gray-600 mt-2"><?= htmlspecialchars($row['category_name']) ?></p>
                <p class="font-semibold text-pink-600 mt-2">$<?= $price ?></p>
              </a>
            </div>
          <?php endwhile; ?>
        <?php else: ?>
