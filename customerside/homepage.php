...
<nav class="bg-pink-100 shadow-md">
  ...
  <!-- Navigation Links -->
  <ul class="flex space-x-6 text-sm md:text-base text-pink-600">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
    <?php while($row = $result->fetch_assoc()): ?>
      <div class="bg-white rounded-xl shadow-md p-4 transition-transform transform hover:scale-105 hover:shadow-xl">
        <img src="<?= $row['image_url'] ?>" alt="<?= $row['product_name'] ?>" class="rounded-md mb-4 w-full h-48 object-cover transition-transform duration-300 ease-in-out transform hover:scale-110">
