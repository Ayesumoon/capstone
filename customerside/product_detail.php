        ...
        <div class="flex items-center gap-4 text-pink-600">
            <a href="cart.php" class="hover:text-pink-500 relative" title="Cart">
                ...
                <div x-show="cartCount > 0"
                     class="absolute top-0 right-0 bg-pink-600 text-white text-xs rounded-full px-2 py-1">
                ...
                <?php if ($isLoggedIn): ?>
                    <div x-data="{ open: false }" class="relative">
                        ...
                        <div x-show="open" @click.away="open = false"
                            ...
                            <a href="profile.php"
                               class="block px-4 py-2 text-sm text-gray-700 hover:bg-pink-100">My Profile</a>
                            <a href="purchases.php"
                               class="block px-4 py-2 text-sm text-gray-700 hover:bg-pink-100">My Purchases</a>
                            <form action="logout.php" method="POST">
                                <button type="submit"
                                        class="w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-pink-100">
                                    Logout
                                </button>
                            </form>
                        </div>
        ...
        <div class="ml-8 flex-1">
            ...
            <div class="mt-6">
                <form id="addToCartForm" method="POST" action="add_to_cart.php">
                    ...
                    <input type="hidden" name="price" value="<?= $product['price_id'] ?>">
                    <input type="hidden" name="product_id" value="<?= $product_id ?>">
                    <input id="quantity_<?= $product_id ?>" type="number" name="quantity" value="1" min="1"
                           class="border rounded px-2 w-16">
                    <br><br>
                    <button type="button"
                            onclick="addToCart(<?= $product_id ?>, '<?= htmlspecialchars($product['product_name']) ?>', <?= $product['price_id'] ?>, document.getElementById('quantity_<?= $product_id ?>').value)"
                            class="bg-pink-500 text-white px-6 py-3 rounded-full text-xl shadow-md hover:bg-pink-600 transition">
                        Add to Cart
                    </button>
                </form>
            </div>
